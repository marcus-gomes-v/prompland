...
export default function FinanceStudentTable({user}: {user: iUser}) {
          ...
          {transactions.map((transaction) => (
            transaction.data.status == 'paid' ? 
              <tr key={transaction.id}>
                ...
                {user.type === 'teacher' &&
                  <td className="whitespace-nowrap px-2 py-2 text-sm text-gray-500">{(transaction.data.amount * 0.1) / 100}</td>
                }
